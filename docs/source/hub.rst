...
If you are hosting multiple ``ExAIs`` it becomes convenient 
...
2. Keeping ExAIs up and running for all models in production 
   in a single place

You can initialize an ``ExplainerHub`` by passing in a list of ``ExAIs``::
...
the ``title`` and ``description`` of each ``ExAI``, and set the
url path with ``name``::

    db1 = ExAI(explainer1, title="Model One", name="db1", 
                description="This is model option one")
    db2 = ExAI(explainer2, title="Model Two", name="db2", 
...
If you had defined users in your ``ExAI`` then these get automatically
carried over to the`` ExplainerHub``::

    db1 = ExAI(explainer1, logins=[['user1', 'password1']])
    db2 = ExAI(explainer2, logins=[['user2', 'password2']])
    hub = ExplainerHub([db1, db2])
...
.. autoclass:: ExAI.dashboards.ExplainerHub
